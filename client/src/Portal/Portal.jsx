    ...
    return(
        <div className="Portal">
            <form className="portalForm" action={sol === 'login' ? 'users/login' : 'users/signup'}>
                ...
                <label>Email</label>
                <input type="text" onChange={(e)=>{setEmail(e.target.value)}} placeholder={'example@control.com'} value={email}></input>
                <label>Password</label>
                <input type="password"  onChange={(e)=>{setPassword(e.target.value)}}></input>
                {sol === 'signup' && <label>Confirm password</label>}
                {sol === 'signup' && <input type="password"  onChange={(e)=>{setPassword(e.target.value)}}></input>}
                <div className="portalButtons">
                    <Button text={sol ==='login' ? 'Login':'Sign Up'} onClick={formSubmitHandler}/>
                    <a href='#' className="textLink" onClick={solSwitchHandler}>{sol === 'login' ? "Don't have an account?" : 'Already got an account?'}</a>
                </div> 
            </form>
        </div>
    )
