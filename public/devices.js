...
const addConfig = function (config) {
  ...
  <span class="favourite-icon ce-fav-icon material-icons ${setFavIconStyle(config)}">${setFavIcon(config)}</span>
  ...
  window.addEventListener("click", async (e) => {
    ...
    if (e.target.classList.contains("material-icons")) {
      ...
      if (e.target.classList.contains("control-fav-icon")) {
        parentEl = e.target.closest(".control");
        type = "control";
      }
      if (e.target.classList.contains("ce-fav-icon")) {
        parentEl = e.target.closest(".configListEntry");
        type = "config";
      }

      const currentFavState = checkCurrentFavState(parentEl);
      const newFavState = getNewFavState(currentFavState);
      const updateObj = favouriteObj(type, parentEl, newFavState);
      e.preventDefault();
